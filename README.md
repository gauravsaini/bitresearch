...
1. **WebGPU**: For fast, hardware-accelerated tensor operations and custom WGSL compute shaders (matmul, causal attention, rotary embeddings, etc.).
...
The core inference, shader infrastructure, and WebRTC ring topology are implemented. To make this production-ready, the following roadmap is planned:

1. **Full backward pass on WebGPU** (Critical Blocker)
   - Implement the autograd graph and backward WGSL shaders to compute actual gradients.
...
## Tech Stack
- **GPU Compute**: WebGPU, custom WGSL compute shaders
