  ...
  async init() {
    ...
    tf.tidy(() => {
      ...
      for (let i = 0; i < nLayer; i++) {
        ...
        const veGateInit = useVe ? tf.zeros([GPTModel.VE_GATE_CHANNELS, nKvHead]) : null;
    ...
    return tf.tidy(() => {
      ...
      for (let i = 0; i < nLayer; i++) {
        ...
        if (this.valueEmbeds.has(i) && layer.veGate) {
          ...
          const xGated = current.slice([0, 0, 0], [B, T, GPTModel.VE_GATE_CHANNELS]);
          const flatGated = xGated.reshape([B * T, GPTModel.VE_GATE_CHANNELS]);
